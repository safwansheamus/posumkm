...
class Payment extends CI_Controller {
    ...
    public function succesfully(){
        ...
        $kuynumber = $this->_create_random_number();
        ...
        foreach($this->cart->contents() as $c){
            ...
            $list .= 'Harga (x1): Rp' . number_format($c['price'],0,",",".") . '%0A';
        ...
        $list .= 'Kode Unik: *Rp' .  $kuynumber . '*%0A';
        $list .= 'Final Total: *Rp' . number_format($totalall + $kuynumber,0,",",".") . '*%0A';
        $list .= '-----------------------%0A';
        $list .= '*Nama:*%0A';
        ...
        $this->cart->destroy();
        redirect("https://wa.me/".$nowa."?text=Halo ".$namatoko."! Saya ingin order : %0A%0A " . $list);
    }

    public function _create_random_number()
    {
        $this->load->helper('string');

        $numKUY = random_string('numeric', 2);
        $numberKUY = $numKUY;
        return $numberKUY;
    }
